...
public abstract class AbstractCombo extends Composite {
	...
	/**
	 * Flag indicating that the popup content must be created each time the
	 * combo is dropped
	 */
	...
	public AbstractCombo(Composite parent, int style) {
		...
		int buttonStyle = SWT.NONE;//SWT.ARROW | SWT.DOWN;
		if ((style & SWT.FLAT) != 0)
	 ...
	 * 
	 * @param parent
	 *            The parent Composite that will contain the control
	 * @return The created Control for the popup content
	 */
	protected abstract Control createPopupContent(Composite parent);

	/**
	 * Creates the text widget. Override to change appearance or behavior of the
	 * default text that is created here.
	 * 
	 * @param style
	 *            text style
	...
	/**
	 * This method is called when a SWT.Selection is notify in the popup
	 * content, allowing to update the Text widget content.
	 * 
	...
	/**
	 * Returns <code>true</code> if the receiver's popup is visible, and
	 ...
	 * the combo is dropped. It can be necessary to set this flag to true when
	 * the popup content use to many controls, as the DateChooser.
	 * 
	 * @param createOnDrop
	 ...
	 * 
	 * @param showButtonOnFocus
	 *            <code>true<code> if button must be shown on focus only
	 */
	...
	/**
	 * Updates the visibility of the button in function of the flag and the
	 * focus.
	 */
